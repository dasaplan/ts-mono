...
describe("Generate Integration", () => {
  describe("ts", () => {
    test.each(["generic/api.yml"])("%s", async (spec) => {
      ...
      const bundled = await generateOpenapi(api, out.absolutePath, { clearTemp: true, tempFolder: out.resolve("tmp").absolutePath });
  ...
  describe("all", () => {
    test.each([
      ...
      const outDir = await generateOpenapi(api, out.absolutePath, { clearTemp: true, tempFolder: out.resolve("tmp").absolutePath });
