...
describe("zod 4 test", () => {
  test("discriminatedUnion", () => {
    const A = z.object({ id: z.string(), type: z.literal("A") });
    const B = z.object({ id: z.string(), type: z.literal("B"), b: z.number() });

    const discriminated = z.discriminatedUnion("type", [A, B]);

    // @error: needs constant literal for discriminator
    // const Unknown = z.object({ type: z.string() }).loose();
    // const discriminated = z.discriminatedUnion("type", [A, B, Unknown]);
  ...
  test("nested discriminatedUnion", () => {
    const A = z.object({ id: z.string(), type: z.literal("AB"), ab: z.literal("a") });
    const B = z.object({ id: z.string(), type: z.literal("AB"), ab: z.literal("b"), b: z.number() });
    const C = z.object({ id: z.string(), type: z.literal("CD"), cd: z.literal("c"), c: z.number() });
    const D = z.object({ id: z.string(), type: z.literal("CD"), cd: z.literal("d"), d: z.number() });
    const E = z.object({ id: z.string(), type: z.literal("E"), e: z.string() });

    const ab_union = z.discriminatedUnion("ab", [A, B]);
    const cd_union = z.discriminatedUnion("cd", [C, D]);
    const nested_union = z.discriminatedUnion("type", [E, ab_union, cd_union]);
...
describe("zod 3", () => {
  test("zod vanilla discriminated union won't fail for Union.or(Unknown)", () => {
    ...
    const discriminated = z.discriminatedUnion("type", [A, B]).or(Unknown);
