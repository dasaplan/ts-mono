...
export type ControllerResult<ResponseMap extends { [status: number]: object | unknown } = { [status: number]: object }> =
  | ({ headers?: Record<string, string | number | Array<string>> } & (
      | { [K in WithCommonHttpStatus<keyof ResponseMap>]: { status: K; json: ResponseMap[K] } }[keyof ResponseMap]
      | { [K in WithCommonHttpStatus<keyof ResponseMap>]: { status: HttpErrorStatus<K>; error: Error } }[keyof ResponseMap]
