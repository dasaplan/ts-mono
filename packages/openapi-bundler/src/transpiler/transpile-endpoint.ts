...
export interface TranspileEndpointCtx extends TranspileContext {
  schema(
    name: string,
    oaSchema: oas30.SchemaObject | oas30.ReferenceObject
  ): Schema;
}
...
export module Endpoint {
  export function transpileAll(ctx: TranspileEndpointCtx): Array<Endpoint> {
    return Object.entries(ctx.resolver.root.paths).flatMap(
      ([path, pathItem]) => {
        ...
        return (["get", "put", "post", "delete", "patch"] as const)
          .flatMap((operationName) =>
            _.isDefined(pathItemResolved[operationName])
              ? [{ ...pathItemResolved[operationName], method: operationName }]
              : []
          )
          .map(
            ({
...
function parseParameters(
  parameters: Array<oas30.ParameterObject | oas30.ReferenceObject> | undefined,
  ctx: TranspileEndpointCtx
): Endpoint["parameters"] {
  return (
...
function parseRequestBody(
  requestBody: oas30.RequestBodyObject | oas30.ReferenceObject | undefined,
  ctx: TranspileEndpointCtx
...
function parseContent(
  contentMap: oas30.ContentObject | undefined,
  ctx: TranspileEndpointCtx
  ...
  return Object.entries(contentMap).map(([mediaType, mediaTypeObj]) => {
    ...
    const format =
      contentType.type === "KNOWN" ? mapFormat(contentType.format) : undefined;
    if (_.isNil(schema)) {
...
function parseJsonFromContent(
  contentMap: oas30.ContentObject | undefined,
  ctx: TranspileEndpointCtx
): (Content & { format: "json" }) | undefined {
  const requestContents = parseContent(contentMap, ctx);
  return requestContents?.find(
    (d): d is Content & { format: "json" } => d.format === "json"
  );
}
