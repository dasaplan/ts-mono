...
export function createCommandBundle(program: Command) {
  program
    ...
    .argument(
      "<openapi-spec>",
      "Absolut or Relative filepath from the cwd to the OpenApi root document file"
    )
    .option(
      "-o, --outputFile [outputFile]",
      "Absolut or relative filepath to cwd of the resulting bundled file"
    )
    .option(
      "--disableMergeAllOf",
      "If set, allOf arrays will be left as is after bundling"
    )
    .option(
      "--disableEnsureDiscriminatorValues",
      "If set, discriminator values won't be ensured on every subType"
    )
    .action(
      async (
        spec: string,
        ...
        const postProcessor = createSpecProcessor({
          ...
          ensureDiscriminatorValues:
            options.disableEnsureDiscriminatorValues ?? true,
        });
